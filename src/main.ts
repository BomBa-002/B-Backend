...
try { execFileSync(process.execPath, ['--import', 'tsx', 'src/infrastructure/database/migrate.ts'], { stdio: 'inherit' }); } catch (error) { logger.error('الترحيل فشل، السيرفر هيكمل عادي', { error }); }
...
const shutdown = (signal: string) => { logger.info('إغلاق هادي للسيرفر', { signal }); openApiWatcher?.close(); server.close(() => { sqlite.close(); process.exit(0); }); setTimeout(() => process.exit(1), 10000).unref(); };
